...
- 🎮 **四种玩法**：
  - **自选模式**：先选曲库，再设置题目数（不能超过所选曲库的歌曲数），答完自动结算
  - **无尽模式**：选曲库后直接开始，永不结束，手动“结束本轮”才结算
  - **专辑模式**：从选择的专辑随机出题
  - **混合模式**：从整个歌库随机出题
...
1. 主菜单点击“自选模式”或“无尽模式”；
2. 在“选择曲库”页面选择游戏（或“混合模式”），自选模式还需填写题目数；
3. 随机播放一首歌，从 A / B / C / D 四个选项中选出歌名；
4. 答对按钮变绿，答错按钮变红并高亮正确答案，1.5 秒后进入下一题；
5. 结算页面显示答对数、答错数、正确率与出题统计。
